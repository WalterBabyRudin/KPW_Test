...
def sample_mnist_Tr(n,fract=0.15):
    # sample mnist data to format training set
    # n: number of trainig samples
    # fract: fraction of digit 1
    
    N, D = np.shape(Feature_Tr)
    s_1 = np.random.binomial(n, fract)
    s_0 = n - s_1
    id_Tr = np.random.choice(N, n+s_0, replace=False)
    
    id_X = id_Tr[:n]
    id_Y_0 = id_Tr[n:]
    id_Y_1 = np.random.choice(len(id_digit_1), s_1, replace=False)
    id_digit_1_Tr = id_digit_1[id_Y_1]
    
    X = Feature_Tr[id_X]
    Y_0 = Feature_Tr[id_Y_0]
    Y_1 = Feature_Te[id_digit_1_Tr]
    Y = np.concatenate((Y_0,Y_1), axis=0)
    
    id_Y = np.random.permutation(len(Y))
    Y = Y[id_Y]
    return np.float32(X), np.float32(Y), id_Tr, id_Y_1
    
def sample_mnist_Te(n,ind_Te,ind_Te_1,fract=0.15):
    ...
    s_1 = np.random.binomial(n, fract)
    s_0 = n - s_1
    id_Te = np.random.choice(len(ind_Te), n+s_0, replace=False)
    id_Te = ind_Te[id_Te]
    
    id_X = id_Te[:n]
    id_Y_0 = id_Te[n:]
    
    id_Y_1 = np.random.choice(len(ind_Te_1), s_1, replace=False)
    id_Y_1 = ind_Te_1[id_Y_1]
    
    X = Feature_Tr[id_X,:]
    Y_0 = Feature_Tr[id_Y_0,:]
    Y_1 = Feature_Te[id_Y_1,:]
    Y = np.concatenate((Y_0,Y_1), axis=0)
    id_Y = np.random.permutation(len(Y))
    Y = Y[id_Y]
...
for id_N in range(L_N_hist):
    ...
    for trial in range(n_run):
        ...
        X_Tr, Y_Tr, id_Tr, id_Y_1 = sample_mnist_Tr(nX_Tr,fract=0.15)
        ...
        #### KPW Test
            ...
            alg_KPW = RBCD_KPW.RBCD_kernel(eta=60,sigma=hsigma*sigma2,tau=1e-4,maxiter=100,tol=5e-1,orho=orho)
            Us = alg_KPW.run_RBCD(X_Tr,Y_Tr,d)
        
        ind_Te = np.delete(id_all, id_Tr)
        ind_Te_1 = np.delete(id_digit_1, id_Y_1)
        
        for test_idx in range(n_test):
            randomNum = 7 + id_N*11 + trial * 1111 + test_idx * 125125
            np.random.seed(randomNum)

            X_Te, Y_Te = sample_mnist_Te(nX_Te,ind_Te,ind_Te_1,fract=0.15)
            X_Te = sigmoid(X_Te)
            Y_Te = sigmoid(Y_Te)
            ...
            ### KPW Test
                decision, _, _ = RBCD_KPW.KPW_Test(X_Te, Y_Te, X_Tr, Y_Tr, Us, sigma2, d,reg=40)
                print('Idx: ',test_idx, '\t N: ',N, '\t Decision: ',decision)
