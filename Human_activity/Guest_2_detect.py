...
data_bending = np.load('Guest/data_bending_2.npy')
data_throwing = np.load('Guest/data_throwing_2.npy')
...
# ### ME Test
    ME_opt = {
        'n_test_locs': 2, # number of test locations to optimize
...
### PW Test
    ...
    d = 3
    alg_PW = RBCD_PW.RBCD_PW(eta=eta_reg,tau=0.1,maxiter=200,tol=0.01,silence=True)
...
## ME Test
    ...
    np.save('ME_Guest_stat_hist_2',Stat_hist)
...
# ## PW Test
    ...
    for trial in range(num_trial):
        ...
        PI = sinkhorn.sinkhorn_knopp(a,b,M,reg=eta_reg)
        stat = np.sum(PI*M)
        print('Trial: ',trial, '\t Stat: ',stat)
        Stat_hist[trial] = stat
    np.save('PW_Guest_stat_hist_2',Stat_hist)
...
# NTK Test
    ...
    np.save('NTK_Guest_stat_hist_2',Stat_hist)
    Threshold = np.quantile(Stat_hist, 0.95)
    print(Threshold)
...
# MMD-O Test
    for trial in range(num_trial):
        ...
        print('Trial: ',trial, '\t Stat: ',Stat)
        Stat_hist[trial] = Stat
    np.save('MMDO_Guest_stat_hist_2',Stat_hist)
    Threshold = np.quantile(Stat_hist, 0.95)
    print(Threshold)
    ...
    for trial in range(num_trial):
        ...
        print('Trial: ',trial, '\t Stat: ',stat)
        Stat_hist[trial] = stat
    np.save('KPW_Guest_stat_hist_2',Stat_hist)
...
if input_method == "ME":
    np.save('ME_Guest_Test_Stat_hist_2',ME_Test_Stat_hist)
    ...
    np.save('PW_Guest_Test_Stat_hist_2',PW_Test_Stat_hist)
    ...
    np.save('NTK_Test_Stat_hist_2',NTK_Test_Stat_hist)
    ...
    np.save('MMDO_Test_Stat_hist_2',MMDO_Test_Stat_hist)
    ...
    np.save('KPW_Guest_Test_Stat_hist_2',KPW_Test_Stat_hist)
