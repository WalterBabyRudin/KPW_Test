...
data_bending = np.load('Guest/data_bending.npy')
data_throwing = np.load('Guest/data_throwing.npy')
...
## ME Test
    ...
    np.save('ME_Guest_stat_hist',Stat_hist)
...
# ## PW Test
    ...
    for trial in range(num_trial):
        ...
        print('Trial: ',trial, '\t Stat: ',stat)
        Stat_hist[trial] = stat
    np.save('PW_Guest_stat_hist',Stat_hist)
...
# NTK Test
    ...
    np.save('NTK_Guest_stat_hist',Stat_hist)
    Threshold = np.quantile(Stat_hist, 0.95)
    print(Threshold)
...
# MMD-O Test
    for trial in range(num_trial):
        ...
        print('Trial: ',trial, '\t Stat: ',Stat)
        Stat_hist[trial] = Stat
    np.save('MMDO_Guest_stat_hist',Stat_hist)
    Threshold = np.quantile(Stat_hist, 0.95)
    print(Threshold)
    ...
    for trial in range(num_trial):
        ...
        print('Trial: ',trial, '\t Stat: ',stat)
        Stat_hist[trial] = stat
    np.save('KPW_Guest_stat_hist',Stat_hist)
...
if input_method == "ME":
    np.save('ME_Guest_Test_Stat_hist',ME_Test_Stat_hist)
    ...
    np.save('PW_Guest_Test_Stat_hist',PW_Test_Stat_hist)
    ...
    np.save('NTK_Test_Stat_hist',NTK_Test_Stat_hist)
    ...
    np.save('MMDO_Test_Stat_hist',MMDO_Test_Stat_hist)
    ...
    np.save('KPW_Guest_Test_Stat_hist',KPW_Test_Stat_hist)
