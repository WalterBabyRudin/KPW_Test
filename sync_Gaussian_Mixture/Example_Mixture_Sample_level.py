...
def generate_Gaussian_mixture_H0(N,D,seedNum):
    # input:
    #     N: sample size
    ...
    for i in range(N):
        ...
        uni_rand = np.random.rand()
        if uni_rand < 0.5:
            Y[i, :] = rv_X1.rvs(1)#, random_state=seedNum + i*41 + 5)
        else:
            Y[i, :] = rv_X2.rvs(1)#, random_state=seedNum + i*151 + 92)
...
for id_N in range(L_N_hist):
    ...
    for trial in range(n_run):
        ...
        X_Tr, Y_Tr = generate_Gaussian_mixture_H0(nX_Tr, D, seedNum = randomNum)
        alpha = 0.05
        ...
        #### KPW Test
            ...
            alg_KPW = RBCD_KPW.RBCD_kernel(eta=20,sigma=sigma2*hsigma,tau=1e-2,maxiter=200,tol=5e-2,orho=orho)
        ...
        for test_idx in range(n_test):
            ...
            X_Te, Y_Te = generate_Gaussian_mixture_H0(nX_Te, D, seedNum = randomNum)
...
if input_method == "ME":
    np.save('ME_decision_hist_GaussMixture_Sample_level.npy', ME_decision_hist)
    print(np.mean(np.mean(ME_decision_hist, axis=1), axis=1))

elif input_method == "PW":
    np.save('PW_decision_hist_GaussMixture_Sample_level.npy', PW_decision_hist)
    print(np.mean(np.mean(PW_decision_hist, axis=1), axis=1))

elif input_method == "NTK":
    np.save('NTK_decision_hist_GaussMixture_Sample_level.npy', NTK_decision_hist)
    print(np.mean(np.mean(NTK_decision_hist, axis=1), axis=1))

elif input_method == "MMDO":
    np.save('MMDO_decision_hist_GaussMixture_Sample_level.npy', MMDO_decision_hist)
    print(np.mean(np.mean(MMDO_decision_hist, axis=1), axis=1))

elif input_method == "KPW": 
    np.save('KPW_decision_hist_GaussMixture_Sample_level.npy', KPW_decision_hist)
