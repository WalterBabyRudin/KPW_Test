...
if __name__ == "__main__":
    ...
    for id_D in range(LD_hist):
        ...
        for i in range(D):
            for j in range(D):
                Q_2[i,j] = np.sqrt(2/(D+1)) * np.sin(i*j*np.pi/(D+1))


        Cov_1 = np.eye(D)
        Cov_2 = np.eye(D)
        Cov_2[0,0], Cov_2[1,1], Cov_2[2,2] = 5,5,5
        Cov_2 = Q_2 @ Cov_2 @ Q_2.T
    ...
    if input_method == "ME":
        np.save('ME_decision_hist_G_nondiag_Var_shifted.npy', ME_decision_hist)
        print(np.mean(np.mean(ME_decision_hist, axis=1), axis=1))

    elif input_method == "PW":
        np.save('PW_decision_hist_G_nondiag_Var_shifted.npy', PW_decision_hist)
        print(np.mean(np.mean(PW_decision_hist, axis=1), axis=1))

    elif input_method == "NTK":
        np.save('NTK_decision_hist_G_nondiag_Var_shifted.npy', NTK_decision_hist)
        print(np.mean(np.mean(NTK_decision_hist, axis=1), axis=1))

    elif input_method == "MMDO":
        np.save('MMDO_decision_hist_G_nondiag_Var_shifted.npy', MMDO_decision_hist)
        print(np.mean(np.mean(MMDO_decision_hist, axis=1), axis=1))

    elif input_method == "KPW":
        np.save('KPW_decision_hist_G_nondiag_Var_shifted.npy', KPW_decision_hist)
